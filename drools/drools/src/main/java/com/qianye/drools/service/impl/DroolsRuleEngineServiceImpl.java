...
import com.qianye.drools.util.drool.RuleUtils;
import com.qianye.drools.util.spring.SpringContextUtil;
import com.qianye.drools.model.fact.RuleExecutionObject;
import com.qianye.drools.util.CollectionUtil;
import com.qianye.drools.util.drool.DroolsUtil;
...
public class DroolsRuleEngineServiceImpl implements DroolsRuleEngineService {
    ...
    public RuleExecutionObject excute(RuleExecutionObject ruleExecutionObject, final String scene) {
        try {
            ...
            if (ksession != null) {
                // （？）如果查到session，表示 该规则（drl）文件已经在drools的工作区/工作内存
                // 会话已打开，未关闭
                //直接执行
                return executeRuleEngine(ksession, ruleExecutionObject, scene);
            } else {
                //重新编译规则(生成ksession)，然后执行
    ...
    private RuleExecutionObject executeRuleEngine(KieSession session, RuleExecutionObject ruleExecutionObject, final String scene) {
        try {
            ...
            // 2.2 将 ruleExecutionObject 也插入到规则中，回调使用
            // ？？ 为什么不直接插入object -> 用户自定义类的时候会用
            session.insert(ruleExecutionObject);

            // 3.将规则涉及的所有动作实现类插入到规则中(如果没有，则不处理)
            // 把动作信息导入运行程序（drl文件只是拼接生成字符串）
    ...
    private RuleExecutionObject compileRule(RuleExecutionObject ruleExecutionObject, final String scene) {
        ...
        // 2.引入global全局对象
        // 暂时只设定一个
        droolRuleStr.append("import com.qianye.drools.model.fact.RuleExecutionResult").append(";").append(LINE_SEPARATOR);
        droolRuleStr.append("global RuleExecutionResult _result").append(";").append(LINE_SEPARATOR);
        //将 RuleExecutionObject 也引入进来
        droolRuleStr.append("import com.qianye.drools.model.fact.RuleExecutionObject").append(";").append(LINE_SEPARATOR);
        ...
        // 5.根据实体信息先组合drools的import语句，至此import拼接结束
        this.insertImportInfo(droolRuleStr, entityList, sceneInfo);
        // 6.遍历并拼出每个规则的执行drools串，至此drl拼接结束
        for (BaseRuleInfo ruleInfo : ruleList) {
            StringBuffer ruleTemp = this.getDroolsInfoByRule(ruleInfo);
    ...
    private void insertImportInfo(StringBuilder droolRuleStr, List<BaseRuleEntityInfo> entityList,
        ...
        if (!CollectionUtil.collectionIsNull(actionList)) {
            ...
            //如果有实现类，则把实现类接口拼装进去
            if (implFlag) {
                droolRuleStr.append(IMPORT).append(" ").append("com.qianye.drools.service.DroolsActionService").append(";").append(LINE_SEPARATOR);
            }
        }
    }
}
