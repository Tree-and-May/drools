...
CREATE DATABASE IF NOT EXISTS `drools` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `drools`;
...
INSERT INTO `RULE_ACTION_INFO` (`ACTION_ID`, `ACTION_TYPE`, `ACTION_NAME`, `ACTION_DESC`, `ACTION_CLASS`, `IS_EFFECT`, `CRE_USER_ID`, `CRE_TIME`, `REMARK`) VALUES
	(1, 1, '测试实现类', '测试实现类', 'com.qianye.drools.service.impl.action.TestActionImpl', 1, 1, '2017-07-24 17:12:32', NULL),
	(2, 2, '自身', '测试自身', 'com.qianye.drools.model.fact.TestRule', 1, 1, '2017-07-27 10:07:03', NULL);
...
CREATE TABLE IF NOT EXISTS `RULE_ACTION_PARAM_INFO` (
  `ACTION_PARAM_ID` BIGINT(20) NOT NULL COMMENT '主键' AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `RULE_ACTION_PARAM_VALUE_INFO` (
  `ACTION_PARAM_VALUE_ID` BIGINT(20) NOT NULL COMMENT '主键' AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `RULE_ACTION_RULE_REL` (
  `RULE_ACTION_REL_ID` BIGINT(20) NOT NULL COMMENT '主键' AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `rule_condition_info` (
  `CONDITION_ID` BIGINT NOT NULL COMMENT '主键' AUTO_INCREMENT,
  ...
  `IS_EFFECT` INT NOT NULL DEFAULT '1' COMMENT '是否有效',
  `CRE_USER_ID` BIGINT NOT NULL DEFAULT '1' COMMENT '创建人',
  `CRE_TIME` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `REMARK` VARCHAR(3000) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`CONDITION_ID`),
  KEY `RULE_ID` (`RULE_ID`),
  KEY `CONDITION_NAME` (`CONDITION_NAME`(255))
) ENGINE=INNODB DEFAULT CHARSET=utf8mb3 COMMENT='规则条件信息表';
...
INSERT INTO `RULE_ENTITY_INFO` (`ENTITY_ID`, `ENTITY_NAME`, `ENTITY_DESC`, `ENTITY_IDENTIFY`, `PKG_NAME`, `CRE_USER_ID`, `CRE_TIME`, `IS_EFFECT`, `REMARK`) VALUES
	(1, '测试规则', '测试规则引擎', 'testRule', 'com.qianye.drools.model.fact.TestRule', 1, '2017-07-20 11:41:32', 1, NULL);
...
CREATE TABLE IF NOT EXISTS `RULE_INFO` (
   `RULE_ID` BIGINT NOT NULL COMMENT '主键' AUTO_INCREMENT,
  ...
  `RULE_ENABLED` INT NOT NULL DEFAULT '1' COMMENT '是否启用',
  `IS_EFFECT` INT NOT NULL DEFAULT '1' COMMENT '是否有效',
  `CRE_USER_ID` BIGINT NOT NULL DEFAULT '1' COMMENT '创建人',
  `CRE_TIME` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `REMARK` VARCHAR(3000) DEFAULT NULL COMMENT '备注',
...
) ENGINE=INNODB DEFAULT CHARSET=utf8mb3 COMMENT='规则信息';
